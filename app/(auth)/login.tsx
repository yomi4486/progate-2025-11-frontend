...
export default function LoginScreen() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      const message: string = (error && (error as any).message) || String(error);
      if (message.toLowerCase().includes('email not confirmed') || message.toLowerCase().includes('confirm') || (error?.status === 403)) {
        router.push({ pathname: '/confirm', params: { email } } as any);
        return;
      }
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <TextInput
        ...
        style={styles.input}
      />
      <Button title={loading ? '処理中...' : 'サインイン / サインアップ'} onPress={handleSignIn} disabled={loading || !email || !password} />
