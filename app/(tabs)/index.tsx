...
function RemoteSizedImage({
  ...
  const imageWidth = useCardPadding ? Math.max(0, containerWidth - CARD_PADDING * 2) : containerWidth;

  return (
    <Image
      source={{ uri }}
      style={{ width: imageWidth, height, borderRadius: 8, marginTop: 12, alignSelf: "center" }}
...
function TimelineCard({ item, onOpen }: { item: TimelineItem; onOpen?: (item: TimelineItem) => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { data, error } = await supabase.from("users").select("name, icon_url").eq("id", item.author).single();
  ...
  return (
    <Pressable onPress={() => onOpen?.(item)}>
      <View style={[cardStyles.card, { height: cardHeight, overflow: "hidden" }]} onLayout={(e) => {
        ...
        <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 8 }}>
          {loadingAuthor ? (
            ...
            <>
              {author?.icon_url ? (
                <Image source={{ uri: author.icon_url as string }} style={{ width: 36, height: 36, borderRadius: 18, marginRight: 8 }} />
              ) : (
        ...
        <ThemedText numberOfLines={5} ellipsizeMode="tail">{item.description ?? ""}</ThemedText>

        {item.attachments && item.attachments.length > 0 && containerWidth != null && (
          <RemoteSizedImage uri={item.attachments[0]} containerWidth={containerWidth} maxHeight={maxImageHeight} />
        )}
      </View>
    </Pressable>
  );
...
function TimelineSwiper({ items, onSwipe, onCardLeftScreen, onOpen }: { items: TimelineItem[] } & SwipeHandlers) {
  const { height: windowHeight } = useWindowDimensions();
  ...
  return (
    <View style={[swipeStyles.container, { position: "relative", minHeight: containerMinHeight }]}>
      {stack.map((item, idx) => {
        const zIndex = stack.length - idx;
        const offsetTop = idx * 8;

        return (
          <View key={item.id} style={[swipeStyles.cardWrapper, { zIndex, top: offsetTop }]}>
            <TinderCard
...
export default function HomeScreen() {
  ...
  const fetchTimelines = useCallback(async () => {
    ...
    try {
      ...
      if (!user) {
        const { data, error } = await supabase.from("timelines").select("*").order("created_at", { ascending: false });
        if (error) throw error;
        setItems((data as unknown as TimelineItem[]) || []);
        return;
      }

      const { data: myLikes, error: likesError } = await supabase.from("likes").select("timeline_id").eq("user_id", user.id);
      if (likesError) throw likesError;
      const swipedIds = myLikes?.map((l) => l.timeline_id) || [];

      let query = supabase.from("timelines").select("*").order("created_at", { ascending: false });
  ...
  useEffect(() => {
    ...
    return () => { try { channel.unsubscribe(); } catch (_) { /* ignore */ } };
  }, []);
  ...
  const handleSwipe = async (id: string, direction: string) => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;
      const { error } = await supabase.from("likes").insert({ user_id: user.id, timeline_id: id, type });
      if (error) { if (error.code !== "23505") console.error("スワイプ保存エラー", error); } else { console.log(`Saved ${type} for post ${id}`); }
    } catch (e) { console.error(e); }
  };

  const handleCardLeft = (id: string) => { console.log("card left screen completely", id); };
  ...
  const CARD_PADDING = typeof _flattenedCard?.padding === "number" ? _flattenedCard.padding : 16;
  ...
  const openCreate = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("users").select("id").eq("id", user.id).limit(1);
      if (error) throw error;
      const exists = Array.isArray(data) ? data.length > 0 : !!data;
      if (!exists) { Alert.alert("プロフィールが必要", "投稿にはプロフィールが必要です。プロフィールを設定しますか？", [{ text: "いいえ", style: "cancel" }, { text: "設定する", onPress: () => router.push("/settings") }]); return; }
      setModalVisible(true);
    } catch (e: unknown) { if (e instanceof Error) { console.error(e); Alert.alert("エラー", e.message || String(e)); } } finally { setCheckingProfile(false); }
  };
  const closeCreate = () => setModalVisible(false);

  const openPost = (item: TimelineItem) => { setSelectedItem(item); setPostModalVisible(true); };
  const closePost = () => { setSelectedItem(null); setPostModalVisible(false); };
  ...
  const handleSubmit = async (title: string, description: string, imageUrls: string[] | null) => {
    ...
    const res = await supabase.from("timelines").insert({ title, description, author: userId, attachments: imageUrls });
  ...
  return (
    <ThemedView style={{ flex: 1 }}>
      {checkingProfile && (
        <View style={{ position: "absolute", left: 0, right: 0, top: 0, bottom: 0, zIndex: 2000, backgroundColor: "rgba(0,0,0,0.45)", justifyContent: "center", alignItems: "center" }}>
      ...
      <ParallaxScrollView scrollEnabled={false} headerBackgroundColor={{ light: "#A1CEDC", dark: "#1D3D47" }} headerImage={null}>
        <ThemedView style={[styles.titleContainer, { paddingTop: insets.top, justifyContent: "space-between" }]}>
        ...
        <ThemedView style={[styles.stepContainer, styles.centerFill, { paddingTop: 8 + insets.top, minHeight: 720 }]}>
          {loading ? (
            <ActivityIndicator size="large" />
          ) : (
            <TimelineSwiper items={items} onSwipe={handleSwipe} onCardLeftScreen={handleCardLeft} onOpen={openPost} />
          )}
        </ThemedView>
      </ParallaxScrollView>
      ...
      <Modal visible={postModalVisible} animationType="slide" transparent={true} onRequestClose={closePost}>
        <ThemedView style={{ flex: 1, justifyContent: "center", alignItems: "center", padding: CARD_PADDING + EXTRA_PADDING }}>
          <ThemedView style={{ width: "100%", maxHeight: "90%", borderRadius: 8, padding: CARD_PADDING + EXTRA_PADDING, backgroundColor: "#fff" }}>
            ...
            <ScrollView>
              <View onLayout={(e) => { const w = e.nativeEvent.layout.width; if (!modalInnerWidth) setModalInnerWidth(w); }}>
                {selectedItem?.attachments && selectedItem.attachments.length > 0 && modalInnerWidth != null && (
                  <RemoteSizedImage uri={selectedItem.attachments[0]} containerWidth={modalInnerWidth} maxHeight={Math.round(windowHeight * 0.8)} contain={true} useCardPadding={false} />
                )}
